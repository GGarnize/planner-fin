...
| Tarefa relacionada | Prompts `DX: comandos simples para ambiente Android/telefone` e `bootstrap de máquina Windows nova + pnpm doctor` |
...
Adicionar tooling local versionado para `pnpm doctor`, bootstrap Windows, `pnpm dev:android`, `pnpm dev:android:services`, `pnpm dev:phone`, builds APK emulator/LAN/remote, servidor local de APK e stop seguro por PID.
...
- Instalar software, iniciar Docker, criar AVD, alterar variaveis/recursos do Windows ou aceitar licencas pelo `pnpm doctor`.
...
**Quando** `pnpm doctor` for executado
...
|---|---|---|---|
| Unitario | LAN, URL, remoto invalido, env temporaria, parsing de versoes, dependencias ausentes/incorretas e readiness. | CA-02, CA-03, CA-05 | `pnpm test:dx`. |
...
| Aceitacao manual | Comandos solicitados no prompt. | CA-01 a CA-05 | Registro de execucao ou limitacao ambiental. |
...
| 2026-08-21 | Adicionar bootstrap Windows e `pnpm doctor` somente diagnostico, com Android Studio opcional. | Prompt `bootstrap de maquina Windows nova + pnpm doctor` | Runbook completo, componentes Android exatos e readiness independente. |
...
- [ ] `pnpm doctor` nao altera a maquina e possui testes de dependencias/versoes/readiness.
