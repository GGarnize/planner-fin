...
@ValidatorConstraint({ name: 'transactionDescription' })
...
@ValidatorConstraint({ name: 'transactionNotes' })
...
export class CreateTransactionDto extends RelationsDto {
  @IsIn(TRANSACTION_STATUSES) status!: (typeof TRANSACTION_STATUSES)[number];
...
export class PayTransactionDto {
  ...
  @IsString() @Validate(CivilDateConstraint) paidAt!: string;
}
